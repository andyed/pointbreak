...
for (const spot of SPOTS) {
  for (const arm of ARMS) {
    for (const sim of TIMES) {
      // --hash= appends extra hash params to EVERY arm, so a change that lives
      // inside the #curl branch (e.g. #earn=0) can be A/B'd with the same
      // instrument in one build — the probe_wave_shape.mjs idiom.
