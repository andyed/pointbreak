...
| `month` | `january` … `december`, `card` | `january` | climatological month: sets H₀ to the **p75** swell height typical of that month at CDIP SC116, de-shoaled to deep water (`data/climatology/pp_monthly_ocean.js`). **Size only** — period is seasonless here (14.4–15.2 s every month), and tide/chop/Δf are not in the bulk pull. Mutually exclusive with `day`; an explicit `h0` wins. Percentile named in the HUD. **January is the shipped default** (2026-08-16; a bare URL means January), so `month=card` is the explicit way back to the site card's authored H₀ — measurement rigs pin it to keep the recorded card basis | control |
