...
|---|---|---|
| Clock contract (`#sim=` seed, `speed=0` freeze, setSim + 2 rAF) | **TRUST** | Verified line by line. The review originally confirmed the `parseFloat('0') || 1` falsy trap; it was resolved by routing the hash through the finite, clamped `parseSpeedParam()` parser. The harness still injects exact sample times rather than depending on rAF timing. |
